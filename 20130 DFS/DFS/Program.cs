...
namespace DFS
{
    internal class Program
    {
        ...
        static void DFSRecursion(BinaryTree graph, int target, Node current) 
        {
            ...
            {
                if (current.visitedChildren.Contains(current.children[i]) == false)
                {
                    Console.WriteLine(current.children[i].index);
                    current.visitedChildren.Add(current.children[i]);
                    DFSRecursion(graph, target, current.children[i]);
                }
            }
        ...
        static List<int> DepthFirstSearch(BinaryTree graph, int targetNodeIndex)
        {
            ...
            // We start in root and we want to find a path from root to the target node
            DFSRecursion(graph, targetNodeIndex, currentNode);
